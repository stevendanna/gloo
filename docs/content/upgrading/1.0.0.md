...
- [Example Upgrade Process](#example-upgrade-process)
    ...
    - [Tearing Down Pre-1.0.0 Installation](#tearing-down-pre-100-installation)
        - [Removing Cluster-Scoped RBAC](#removing-cluster-scoped-rbac)
...
\<v0.21.0 or \>=v0.21.0. However, in both cases you may delete the v2 Gateway CRD:
...
And then run the base uninstall, using the saved `glooctl binary`. If you are using Gloo \>=0.21.0, be sure to note
the installation ID that is output during the uninstall.

```shell script
~ > ./glooctl-v0.21.1 uninstall -n gloo-system # make note of the installation ID, if you are on 0.21.0 or above!
Uninstalling Gloo...
...
`glooctl uninstall` will not remove cluster-scoped RBAC. If you are running Gloo 0.21.0 or above, then you may run

```shell script
kubectl delete clusterrole -l app=gloo,installationId=${installation-id-from-above}
kubectl delete clusterrolebinding -l app=gloo,installationId=${installation-id-from-above}
```
